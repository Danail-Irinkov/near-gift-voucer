// @nearfile
import { env, storage, base58, context, u128, PersistentMap, ContractPromise, ContractPromiseBatch, logging } from 'near-sdk-as'

// --- contract code goes below

// It's good to use common constant, but not required.
import { LAST_SENDER_KEY, LAST_SENDER_KEY2, messages } from "./model"
...
export function whoSaidHi(): string | null {
  ...
  return storage.getString(LAST_SENDER_KEY2);
}

export function getVoucher(email:string): string | null {
...
function removeAtSymbolFromEmail(email: string): string {
	return email.replace('@', '__AT2__');
}

function addAtSymbolBackToEmail(email: string): string {
	return email.replace('@', '__AT2__');
}
...
export function create_account_and_claim(new_account_id: AccountId, new_public_key: Base58PublicKey): void {
	...
	ContractPromiseBatch
		.create(new_account_id) //act on new_account_id
